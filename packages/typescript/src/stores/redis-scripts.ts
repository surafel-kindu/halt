 ...
 * decision is atomic even under concurrent access from many app servers. Every
 * script touches exactly one key (KEYS[1]) and reads the clock from the Redis
 * server (`TIME`), so it is safe on Redis Cluster and immune to app-server clock skew.
 *
 ...
 *   { allowed(1|0), limit, remaining, resetAt(unix s), retryAfter(s, -1 = none) }
 */
